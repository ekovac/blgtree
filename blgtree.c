...
struct BLGTree {
	blgnode_t* root;
	blgnode_t* traversal_vector;
	size_t traversal_vector_len;
	size_t traversal_vector_available;
};
...
{
	if (tree->traversal_vector)
		free(tree->traversal_vector);
	blgtree_clear(tree);
	free(tree);
}
